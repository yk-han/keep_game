// Validation
var key_enko_flag = "EN";
var key_result = "";
var key_save_string = "";
var key_kor_string = "";
var key_maxlength = 9999;

/***
 * Key Language Library
 * @param varString : 기존에 입력문자
 * @param varLang : 한글,영문키 구분 (EN: 영어, KO: 한국어)
 * @param varCallback : 최종 결과를 보낼 함수명
 */
function lib_func_keylang_init(varLang, varLength, varString) {	
	// 입력 데이터 설정	
	
	if ( typeof varLang != "undefined" ){ 
		key_enko_flag = varLang;
	}
	if ( typeof varLength != "undefined" ){ 
		key_maxlength = varLength;
	}	
	if ( typeof varString != "undefined" ){ 
		key_result = varString;
		key_save_string = varString;
	}
	
	// 키 입력 이벤트 설정
	window.addEventListener("keydown", function(e) {
		if ( e.keyCode != 8  && e.keyCode != 46 ){	
			if ( key_result.length > key_maxlength ){			
				e.preventDefault();
				return;
			}			
		}	
		if ( e.keyCode != 16 && ((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 96 && e.keyCode <= 105)) ){
			// 숫자 0~9		
			key_save_string += e.key;
			key_result += e.key;
		} else if ( e.keyCode == 16 && ((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 96 && e.keyCode <= 105)) ){
			// 특수문자		
			key_save_string += e.key;
			key_result = key_save_string;
		} else if ( e.keyCode >= 65 && e.keyCode <= 90 ){
			if ( key_enko_flag == "EN" ){
				// 소문자 영문 a-z			
				key_save_string += e.key;
				key_result = key_save_string;
			} else if ( key_enko_flag == "KO" ){
				// 한글 일반			
				key_kor_string += e.key;				
				key_result = key_save_string + engTypeToKor(key_kor_string);
			}
		} else if ( e.keyCode == 16 && (e.keyCode >= 65 && e.keyCode <= 90) ){
			if ( key_enko_flag == "EN" ){
				// 대문자 영문 A-Z				
				key_save_string += e.key.toUpperCase();
				key_result = key_save_string;				
			} else if ( key_enko_flag == "KO" ){
				// 한글 shift			
				key_kor_string += e.key.toUpperCase();				
				key_result = key_save_string + engTypeToKor(key_kor_string);	
			}
		} else if ( e.keyCode == 8 ){
			// Backspace
			if ( key_enko_flag == "EN" ) {
				key_save_string = key_save_string.substring(0, key_save_string.length - 1);				
			} else if (key_enko_flag == "KO"){
				key_save_string = key_save_string + engTypeToKor(key_kor_string);
				key_save_string = key_save_string.substring(0, key_save_string.length - 1);				
				key_kor_string = "";				
			}
			key_result = key_save_string;
			...
			// GameMaker 로 콜백 (한영키 변환)
			lib_func_keylang_lang(key_enko_flag);
		}
		// GameMaker 로 콜백 (String)
		lib_func_keylang_string(key_result);
...
/***
 * Key Button Change Callback
 * @param params : KO/EN 코드
 */
function lib_func_keylang_lang( param ){
	window["gml_Script_gmcallback_keylang_lang"]("", "", param);
}

/***
 * Key Result String
 * @param params : 회신보낼 데이터
 */
function lib_func_keylang_string( param ){
	window["gml_Script_gmcallback_keylang_string"]("", "", param);	
}
